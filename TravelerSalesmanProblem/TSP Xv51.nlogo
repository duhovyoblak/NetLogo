...
globals      [ pocet_agentov       ;; pocet agentov = koeficient_agentov * pocet_miest  
               ...
               pocet_susedov       ;; absolutny pocet susedov
...
salesmans-own[ 
               ...
               fen_iddle           ;; pocet tikov necinnosti
...
;  random-seed new-seed             ;; generuj nove nahodne cisla
  ...
  set pocet_susedov round ( pocet_miest * perc_susedov / 100 )
  if pocet_susedov < 2 [ set pocet_susedov 2]
...
to krok                            ;; jeden krok simulacie
  ...
  ;; smrt necinnostou
  ;;--------------------------------------------------------------------------
  ask salesmans with [ fen_iddle > max_iddle ] [ die ]
...
to fenotyp  ;; vyhodnot fenotyp v kontexte Salesmana
  ...
  ;; adjustacia zony 
  set fen_xmin fen_xmin - zona_adjust * ( fen_xmax - fen_xmin ) / 100
  set fen_xmax fen_xmax + zona_adjust * ( fen_xmax - fen_xmin ) / 100
  set fen_ymin fen_ymin - zona_adjust * ( fen_ymax - fen_ymin ) / 100
  set fen_ymax fen_ymax + zona_adjust * ( fen_ymax - fen_ymin ) / 100
  ...
  set fen_value   fen_T_act * fen_T_act / fen_T_pot / fen_Q_avg
;  set fen_value   100 * fen_T_act / fen_T_pot / fen_Q_avg
...
to optimalizacia ;; optimalizuj obchodnu trasu kontext Salesman
  ...
  foreach gen_path
  [
    ;; Mesta, tvoriace spracovavany join gen_path-u
    let TA [end1] of ?
    let TB [end2] of ?
    ...
    if any? opttowns [
      ...
      if ( [quality] of JAO + [quality] of JOB ) <= ( 2 * [quality] of JAB ) [
...
;  let source other salesmans with [ non_prekryv fen_xmin fen_xmax [fen_xmin] of myself [fen_xmax] of myself 
;                                    or
;                                    non_prekryv fen_ymin fen_ymax [fen_ymin] of myself [fen_ymax] of myself ]
...
to-report prekryv[ amin amax bmin bmax ]
  ...
  ifelse amin <= bmin
    ...
    [ ifelse ( amax - bmin ) / lb * 100 > 60 [report True] [report False] ]
    [ ifelse ( bmax - amin ) / lb * 100 < 60 [report True] [report False] ]
...
to model_show
  ...
  if zobraz = "Selected"        [ selected_show ]
...
true
true
"set-plot-x-range 1 round ( pocet_susedov / 2)\nset-plot-y-range 0 round ( count salesmans / 4 ) + 1\n\nset-current-plot-pen \"Quality\"\nset-histogram-num-bars 20" ""
PENS
"Quality" 1.0 1 -16777216 true "" "histogram [fen_Q_avg ] of salesmans"
...
Zobraz
Zobraz
"Joins" "Best value" "Max towns path" "Top 5 winners" "Shortest path" "Selected"
4
